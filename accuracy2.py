...
def get_data(fp_obs, fp_true_net, K, num_nodes):
...
def get_accuracy2(fp_obs_o, fp_obs_e, fp_true_net_o, fp_true_net_e, K, num_nodes):
    ...
    for i in range(len(data_o)):
        ...
        if (data_o[i] == data_e[i]).all():
            ...
            xnear, ynear, n1near, n2near, tnear = find_near(data_e[i], data_o)
            x_near.append(xnear)
            y_near.append(ynear)
    ...
    accuracy_n1 = cal_accuracy(n1, n1_near)
    accuracy_n2 = cal_accuracy(n2, n2_near)
